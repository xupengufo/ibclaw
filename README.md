...
用 **IB Gateway**（IBKR 官方桌面应用）+ **ib_async**（Python socket API）替代了整套 HTTP 方案：
...
| 断线恢复 | Selenium 自动登录（不可靠） | ib_async 内置自动重连 |
| 依赖项 | requests, urllib3, selenium, chromedriver | **ib_async + requests** |
...
安装完成后，请帮我配置 IB Gateway + ib_async 环境。
```
...
```bash
# 在仓库根目录执行（推荐）
# 会创建 ~/trading/venv，安装 ib_async + requests，
...
应该能看到网关连接成功的状态，以及账户余额、持仓、行情等数据。
...
| 自动投研链路 (SOP) | ✅ | 内置个股诊断、短线雷达、大盘扫描 3 大自动化 AI 推理流程 |
| JSON-First 协议 | ✅ | 核心查询均支持 `--json` 输出，消除大模型长文本解析幻觉 |
| 下单 | ❌ | **完全不支持** |
| 修改/取消订单 | ❌ | **完全不支持** |
...
│   ├── vix_dashboard.py     # 😱 VIX 恐慌指数仪表盘
│   ├── exit_advisor.py      # 🎯 智能止盈止损建议器
│   ├── stock_compare.py     # ⚔️ 股票对比分析
│   ├── risk_budget.py       # 🏛️ 风险预算计算器
│   ├── options_flow.py      # 🔍 期权异常活动扫描
│   ├── daily_report.py      # 📋 持仓日报生成器
...
│       └── SOP-portfolio-checkup.md  # 组合全面体检
└── references/
