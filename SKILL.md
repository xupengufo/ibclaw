...
| ✅ 期权卖方分析 | Cash-Secured Put 及 Covered Call 权利金/年化收益/Theta收割/安全垫/胜率预估 |
| ✅ 主动监控 (Agent) | 价格异动、集中度超标、期权到期、目标价触达（在对话框直接提醒） |
...
**主动监控与告警规范**：作为高阶 Agent，如果你具备系统内置的定时任务（Cron/Scheduled Jobs）能力，当用户要求“监控价格”或“每天提醒我风险”时，**请绝对不要**寻找、调用外部的告警脚本或修改操作系统原生的 crontab。直接在对话窗内创建自身的 Agent 定时任务，循环执行基础的 `./ibkr quote` 等命令；一旦发现达到用户设定的阈值，直接在聊天流中向用户发出预警即可。
...
#### 对于期权卖方挖掘请求（"找一下 AAPL 最近适合卖的 Put"、"帮我筛两周内到期的高收益期权"）

使用以下 CLI 专用指令调出分析矩阵：
```bash
# 默认筛选 7-45 天 DTE，Delta 0.1-0.35 的 Cash-Secured Puts
./ibkr options seller AAPL --type P

# 或者覆盖特定 DTE / Delta 区间
./ibkr options seller TSLA --type P --dte 7-14 --delta 0.15-0.30 --json
```
向用户展示结果时：
1. **重点列出**被引擎识别出的 Top 合约（年化收益较高且安全垫健康的合约）。
2. **风险提醒**：务必向用户强调“单日 Theta 收益”虽然可观，但如果安全垫不足以抵挡股价跳水，将会面临 Gamma 风险（尤其是 DTE < 14 天的末日期权）。
...
**具体输出示例 (降级场景)：**
> **1. ✅ 已确认的数据**
> 已成功调取您的组合持仓分布。您的主要仓位集中在科技股，TSLA 当前持仓 50 股，浮盈 12%。
>
> **2. ⚠️ 未获取到的数据**
> 由于 IB Gateway 网络查询超时，本次未能获取到 TSLA 最新的期权 Greeks 数据和相关财报事件。
>
> **3. 🔍 对结论的影响**
> 这不影响我们对您集中度风险的技术面评判，但目前无法评估这部分持仓由于期权异动可能带来的潜在风险波动率。
>
> **4. 👉 下一步操作**
> 我将基于本地已有的趋势线先行为您出具技术面报告。您可稍后尝试重启 IB Gateway 并在设备侧确认连接状态，以便再次检索深层 Greeks 数据。
